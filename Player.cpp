...
Player::Player(const int ID,Team* team, int gamesPlayed, int goals, int cards, const bool gkFlag)  :
        m_playerId(ID), m_team(team), m_gamesPlayed(gamesPlayed), m_teamsGamesWithout(0), m_goals(goals), m_cards(cards), m_gkFlag(gkFlag), m_closestAbove(nullptr), m_closestBelow(nullptr){}
...
void Player::setTeam(Team* newTeam) {
    m_team = newTeam;
    m_teamsGamesWithout = m_team->getGames();
}
...
int compareGoals(const Player &p1, const Player &p2){
...
bool compareIdPlayers(const Player &p1, const Player &p2){
